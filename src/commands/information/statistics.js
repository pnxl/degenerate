...
module.exports = {
  ...
  async execute(interaction) {
    ...
    function bootMode() {
      const parsedBoot = hostinfo.os.sys.uefi.toString()
        .replace('true', 'UEFI')
        .replace('false', 'CSM/Legacy')
      
      if (hostinfo.os.sys.distro === "macOS" || hostinfo.os.sys.distro === "Mac OS X") return parsedBoot.replace('UEFI', 'Apple EFI')
      return parsedBoot
    }
    ...
    const board = new MessageEmbed()
      ...
      .addField("virtualised", hostinfo.sys.virtual.toString().replace("true", "yes").replace("false", "no"), true)
      .addField("hackintosh", hackintosh().toString().replace("true", "yes").replace("false", "no"), true)
    ...
    const cpu = new MessageEmbed()
      ...
      .addField("num of CPUs", hostinfo.cpu.processors.toString(), true)
      .addField("cores", hostinfo.cpu.physicalCores.toString(), true)
      .addField("threads", hostinfo.cpu.cores.toString(), true)
      .addField("supported features", "```\n" + hostinfo.cpu.flags.replaceAll(' ', ', ') + "\n```")
    
    const mem = new MessageEmbed()
      .setTitle(`📋 memory`)
      .setColor(cfg.embed.colours.default)
      .addField("total memory", hostinfo.mem.info.total / 1073741824 + " gigabytes", true)
      .addField("type", hostinfo.mem.layout[0].type, true)
      .addField("clock speed", hostinfo.mem.layout[0].clockSpeed + " MHz", true)
      .addField("error-correcting", hostinfo.mem.layout[0].ecc.toString().replace(true, "yes").replace(false, "no"), true)
      .addField("form-factor", hostinfo.mem.layout[0].formFactor || "???", true)
      .addField("slot", hostinfo.mem.layout[0].bank, true)
      .addField("voltage", hostinfo.mem.layout[0].voltageConfigured || "???", true)
      .addField("min voltage", hostinfo.mem.layout[0].voltageMin || "???", true)
      .addField("max voltage", hostinfo.mem.layout[0].voltageMax || "???", true)
    ...
    const gfx = new MessageEmbed()
      ...
      .addField("dynamic vram", hostinfo.gfx.controllers[0].vramDynamic.toString().replace("true", "yes").replace("false", "no"), true)
      .addField("video memory", hostinfo.gfx.controllers[0].vram + " MB", true)
      .addField("metal version", hostinfo.gfx.controllers[0].metalVersion || "not on macOS", true)
    ...
    const ops = new MessageEmbed()
      ...
      .addField("codename", hostinfo.os.sys.codename.replace('macOS ', '').replace('Mac OS X ', '') || "???", true)
      ...
      .addField("service pack", hostinfo.os.sys.servicepack || "not on Windows", true)
      .addField("under hyper-v", hostinfo.os.sys.hypervizor || "not on Windows", true)
      .addField("through rdp", hostinfo.os.sys.remoteSession || "not on Windows", true)
    ...
    const disks = new MessageEmbed()
      ...
      .addField("size", Math.floor(hostinfo.disk[0].size / 1073741824) + " GB" || "???", true)
      .addField("device", hostinfo.disk[0].device || "???", true)
      .addField("type", hostinfo.disk[0].type.replace('HD', 'Hard Disk Drive').replace('SSD', 'Solid State Drive').replace('NVMe', 'Solid State Drive over NVMe'), true)
      ...
      .addField("S.M.A.R.T. status", hostinfo.disk[0].smartStatus.replace('unknown', '???') || "???", true)
      ...
      const usb = new MessageEmbed()
        ...
        .addField("removable", hostinfo.usb[0].removable.toString().replace("true", "yes").replace("false", "no"), true)
    ...
    if (opt === "nerd") return interaction.reply({ embeds: [board, cpu, mem, gfx, ops, sw, disks, usb] });
